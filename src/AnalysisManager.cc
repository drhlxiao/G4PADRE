...
void AnalysisManager::ProcessStep(const G4Step *aStep) {
	...
	if (volName.contains("blackhole")) { // don't store too many tracks
		aStep->GetTrack()->SetTrackStatus(fKillTrackAndSecondaries);
		if (eventID % 10000 == 0) {
			G4cout << "WARNING: Tracks are killed in material "<<volName<< "as its material  defined as blackhole! "<<G4endl;
		}
		numKilled++;
	}
	...
	if (volName == "pixel") {
		...
		hpat[detectorID]->Fill(pixelID);


		
		if(killTracksEnteringDetectors) {
			edep = aStep->GetPreStepPoint()->GetKineticEnergy()/keV;
			aStep->GetTrack()->SetTrackStatus(fKillTrackAndSecondaries);
			if (eventID % 1000 == 0) {
				G4cout << "WARNING: Killing tracks in Pixels! "
					<< G4endl;
			}
			numKilled++;
			FillDetectorIncidentParticle(aStep);
			//G4cout<<"Filling:"<<detectorID<<" "<<pixelID<<G4endl;
			//kill tracks in it
		}
	...
	if (nextVolName == "Caliste" && preVolName != "Caliste") {
		boundary = 2;
	} 
	//if (nextVolName != "Caliste" && preVolName == "Caliste") {
	//  boundary = boundary &0x8;
	//}


	if (boundary == 0) return;
	// record tracks entering pixels or caliste
	...
	if(boundary==1)
	{
		...
		detectorID= postStep->GetTouchable()->GetCopyNumber(0);
		pixelID=0;
	}
	...
	if(!killTracksEnteringDetectors){
		FillDetectorIncidentParticle(aStep);
		//don't fill again otherwise it will fill twice
	}
